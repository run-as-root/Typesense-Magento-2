/**
 * Typesense Landing Page Search Alpine.js component
 *
 * Plain function — no RequireJS. Loaded via standard <script> tag.
 * Registered as an Alpine.js component via x-data="typesenseLandingPageSearch(config)".
 *
 * Differences from category-search.js:
 * - query uses config.query (landing page's configured query, defaults to '*')
 * - filter_by uses config.filterBy as a base filter (pre-applied from landing page config)
 * - sort_by uses config.sortBy as default sort (pre-applied from landing page config)
 * - User can still apply additional facet filters on top of the base filter
 */
function typesenseLandingPageSearch(config) {
    return {
        query: config.query || '*',
        hits: [],
        facets: [],
        ...
        async search() {
            ...
            // Build filter_by: start with the landing page's base filter, then layer user facets on top
            const filterParts = [];

            if (config.filterBy) {
                filterParts.push(config.filterBy);
            }
